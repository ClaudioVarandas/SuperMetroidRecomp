...
/* Output path — overwritten per dump. Relative to the launch directory
 * (the repo root; the binary is run as `./build/<exe> <rom>`), so it is
 * not machine-specific. (Was an absolute SuperMarioWorldRecomp path
 * copied from the SMW scaffold.) */
static const char *kReportPath = "build/last_run_report.json";
